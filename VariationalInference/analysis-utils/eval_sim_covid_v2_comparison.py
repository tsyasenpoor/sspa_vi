#!/usr/bin/env python
"""Aggregate sim_covid_v2 easy/medium/hard 4-method x 5-seed comparison.

Usage:
    python eval_sim_covid_v2_comparison.py {easy|medium|hard}

v2: all variants are cell-level (overlap-based difficulty, no patient labels).
Per-cell test AUC only.

Outputs comparison_table.csv + comparison_summary.csv under
/labs/Aguiar/SSPA_BRAY/results/sim_covid_v2/sim_{variant}/comparison/.
"""
import argparse
import json
import sys
...
def main():
    ...
    data_dir    = Path(f"/labs/Aguiar/SSPA_BRAY/scdesign3_covid19_1kcells_2kgenes/sim_v2/{variant}")
    results_dir = Path(f"/labs/Aguiar/SSPA_BRAY/results/sim_covid_v2/sim_{variant}/comparison")
    ...
    patient_auc_fn = None  # v2: cell-level labels only
