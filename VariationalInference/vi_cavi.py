...
8. Unmasked mode only -- no pathway masking
9. Factor rescaling for identifiability at each iteration
...
class CAVI:
    ...
    b_v : float
        Laplace prior scale for v (regression weights, Bayesian Lasso).
    ...
    def _initialize(self, X, y, X_aux):
        ...
        # Scale b_v so prior precision is meaningful relative to data precision.
        # Following Park & Casella (2008): b_v_eff = b_v * sqrt(K) / sqrt(N)
        b_v_eff = self.b_v * np.sqrt(K) / np.sqrt(self.n)
        print(f"  [Laplace] b_v auto-scaled: {self.b_v:.4f} -> {b_v_eff:.6f} "
              f"(N={self.n}, K={K})")
        self.b_v = b_v_eff
        # Init sigma_v_diag (tau^2) from Laplace variance = 2*b_v^2
    ...
    def _rescale_factors(self):
        """Rescale theta and beta per factor for identifiability (pseudocode step 6).

        c_l = sqrt(mean_i E[theta_il] * mean_j E[beta_jl]) / mean_i E[theta_il]
        b_theta_il /= c_l
        b_beta_jl *= c_l
        mu_v_kl /= c_l
        tau2_v_kl /= c_l^2

        This balances E[theta] and E[beta] to their geometric mean per factor
        while keeping theta*beta and theta*v invariant.
        """
        for k in range(self.K):
            ...
            if USE_JAX:
                ...
                self.b_beta = self.b_beta.at[:, k].set(self.b_beta[:, k] * c_k)
                self.mu_v = self.mu_v.at[:, k].set(self.mu_v[:, k] / c_k)
                self.sigma_v_diag = self.sigma_v_diag.at[:, k].set(
                    self.sigma_v_diag[:, k] / (c_k ** 2))
            else:
                self.b_theta[:, k] /= c_k
                self.b_beta[:, k] *= c_k
                self.mu_v[:, k] /= c_k
    ...
    def _update_beta(self, z_sum_beta):
        """Beta update (pseudocode step 2).

        a_beta_jl = alpha_beta + sum_i E[z_ijl]
        b_beta_jl = E[eta_j] + sum_i E[theta_il]
        """
    ...
    def _update_eta(self):
        """Eta update (pseudocode step 3).

        a_eta_j = alpha_eta + K * alpha_beta   (constant, set in init)
        b_eta_j = lambda_eta + sum_l E[beta_jl]
        """
        self.b_eta = self.dp + self.E_beta.sum(axis=1)

    def _update_theta(self, z_sum_theta, y, X_aux):
        """Theta update (pseudocode step 4).

        a_theta_il = alpha_theta + sum_j E[z_ijl]
        b_theta_il = E[xi_i] + sum_j E[beta_jl] + omega * R_il

        R_il has linear + quadratic parts in E[theta]; the quadratic
        leads to solving b^2 - b_base*b - c_quad*a = 0.

        Row-chunked to avoid multiple full (n, K) intermediates.
        """
    ...
    def _update_zeta(self, X_aux):
        """Zeta update (pseudocode step 7).

        zeta_ik = sqrt(E[A_ik^2])
        E[A_ik] = E[theta_i]^T mu_v_k
        E[A_ik^2] = E[A_ik]^2 + sum_l Var(theta_il) * E[v_kl^2]
                     + sum_l E[theta_il]^2 * tau^2_v_kl
    ...
    def _update_v(self, y, X_aux):
        """V update — Bayesian Lasso (pseudocode step 8).

        omega_kl = sqrt(mu_v_kl^2 + tau^2_v_kl)
        E[s_kl^-1] = 1/(b_v * omega_kl) + 1/omega_kl^2
        tau^-2_v_kl = E[s_kl^-1] + 2 * sum_i lambda(zeta_ik) * E[theta^2_il]
        mu_v_kl = tau^2_v_kl * sum_i [(y_ik - 0.5) * E[theta_il]
                  - 2*lambda(zeta_ik) * E[theta_il] * C_ik^(-l)]

        Memory-efficient: row-chunked accumulation of (kappa, K) sufficient
        ...
        # Laplace prior precision from inverse-Gaussian scale mixture
        E_v_sq = self.mu_v ** 2 + self.sigma_v_diag
        omega = xp.sqrt(xp.maximum(E_v_sq, 1e-12))
        prior_precision = 1.0 / (self.b_v * omega) + 1.0 / (omega ** 2)

        # Accumulate sufficient statistics in row chunks
        min_chunk = max(1024, self.n // 256)
        prec_sum = xp.zeros((self.kappa, self.K))
        term1_sum = xp.zeros((self.kappa, self.K))
        parta_sum = xp.zeros((self.kappa, self.K))
        partb_sum = xp.zeros((self.kappa, self.K))
        ...
        while i0 < self.n:
            ...
            try:
                b_theta_c = self.b_theta[i0:i1]
                E_theta_c = self.a_theta[i0:i1] / b_theta_c
                E_theta_psq_c = xp.square(E_theta_c)
                Var_theta_c = E_theta_c / b_theta_c
                E_theta_sq_c = E_theta_psq_c + Var_theta_c
        ...
        precision = prior_precision + 2 * prec_sum
        term1 = term1_sum
    ...
    def _compute_elbo(self, X_dense, y, X_aux):
        ...
        # === Prior: p(v) — Laplace (Bayesian Lasso) ===
        E_v_sq = self.mu_v ** 2 + self.sigma_v_diag
        omega = xp.sqrt(xp.maximum(E_v_sq, 1e-12))
    ...
    def fit(self, X_train, y_train, X_aux_train=None,
            ...
            verbose=True, early_stopping='heldout_ll', **_ignored):
        """
        ...
        for t in range(max_iter):
            ...
            # 2b. Rescale factors for identifiability (pseudocode step 6)
            ...
            if diag:
                ...
                print(f"  [diag t={t}] after theta,xi: "
                      ...
                      f"a_theta=[{float(self.a_theta.min()):.4e},{float(self.a_theta.max()):.4e}]")
    ...
    def _infer_theta_sparse(self, X_coo, n_new, n_iter=20, X_aux_new=None):
        ...
        for _ in range(n_iter):
            ...
            if has_regression:
                ...
                zeta_new = xp.sqrt(xp.maximum(E_A_sq, 1e-8))
